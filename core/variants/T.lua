...
variant_t.on_init = function()
    ...
    variant_t.rooms = {
        { func = variant_t.tons_of_rocks, weight = 5, min_discovered_rooms = 0,  max_discovered_rooms = 0, guaranteed_at = { 1 } },
        { func = variant_t.tons_of_trees, weight = 1, min_discovered_rooms = 0,  max_discovered_rooms = 0, guaranteed_at = { 2 } },
    }
end
...
--- @return nil
variant_t.init_cell = function(positions)
    ...
    global.discovered_cells = global.discovered_cells + 5
end
...
--- @param positions table - Table of positions as a coords of left top corner of the chunk (room)
--- @return nil
variant_t.tons_of_rocks = function(surface, positions)
    for _, position in pairs(positions) do
...
--- @param positions table - Table of positions as a coords of left top corner of the chunk (room)
--- @return nil
variant_t.tons_of_trees = function (surface, positions)
    for _, position in pairs(positions) do
        local left_top = { x = position.x * global.config.grid_size, y = position.y * global.config.grid_size }
...
--- @return boolean
variant_t.can_expand = function (position, direction)
    for _, offset in pairs(variant_t.get_offsets(position, direction)) do
        if variant_t.can_expand_offset(offset) then
            return true
        end
    end
...
--- @return boolean
variant_t.can_expand_offset = function (offset)
...
--- @param direction defines.direction - Direction in which the room will be placed
--- @return table
variant_t.get_random_expandable_positions = function (position, direction)
    local available_positions = {}
    
    for _, offset in pairs(variant_t.get_offsets(position, direction)) do
        local expandable_positions = {}
        for _, offset_pos in pairs(offset) do
            if variant_t.can_expand(offset_pos, direction) then
...
--- @param direction defines.direction - Direction in which the room will be placed
--- @return table
variant_t.get_offsets = function (position, direction)
    local offsets = {}

    local directions = {
        [defines.direction.north] = {{{ 0, -1 }, { 0, -2 }, { -1, -2 }, { 1, -2 }}, {{ 1, 0 }, { 2, 0 }, { 2, -1 }, { 2, 1 }}, {{ -1, 0 }, { -2, 0 }, { -2, -1 }, { -2, 1 }}, {{ 0, -1 }, { 0, -2 }, { -1, 0 }, { 1, 0 }}},
        [defines.direction.south] = {{{ 0, 1 }, { 0, 2 }, { -1, 2 }, { 1, 2 }}, {{ -1, 0 }, { -2, 0 }, { -2, 1 }, { -2, -1 }}, {{ 1, 0 }, { 2, 0 }, { 2, 1 }, { 2, -1 }}, {{ 0, 1 }, { 0, 2 }, { -1, 0 }, { 1, 0 }}},
        [defines.direction.east] = {{{ 1, 0 }, { 2, 0 }, { 2, -1 }, { 2, 1 }}, {{ 0, 1 }, { 0, 2 }, { -1, 2 }, { 1, 2 }}, {{ 0, -1 }, { 0, -2 }, { -1, -2 }, { 1, -2 }}, {{ 1, 0 }, { 2, 0 }, { 0, -1 }, { 0, 1 }}},
        [defines.direction.west] = {{{ -1, 0 }, { -2, 0 }, { -2, 1 }, { -2, -1 }}, {{ 0, -1 }, { 0, -2 }, { -1, -2 }, { 1, -2 }}, {{ 0, 1 }, { 0, 2 }, { -1, 2 }, { 1, 2 }}, {{ -1, 0 }, { -2, 0 }, { 0, -1 }, { 0, 1 }}},
    }
